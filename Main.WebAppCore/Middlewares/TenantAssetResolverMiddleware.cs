...
public class TenantAssetResolver: ITenantAssetResolver
{
    ...
    public string GetLogoUrl ()
    {
        ...
        if ( _tenantSetter.CurrentTenant.ThemeModel?.LogoFileName == null )
        {
            return "~/favicon.ico";
        }

        // Return the clean, resolved path
        return $"~/uploads/{_tenantSetter.ResolvedTenantId.ToString ()}/logos/{_tenantSetter.CurrentTenant.ThemeModel?.LogoFileName!}";
    }
}
